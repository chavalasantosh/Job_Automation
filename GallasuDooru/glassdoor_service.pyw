"""
Glassdoor Full Automation Service (.pyw)
========================================
...
LOCK_FILE = os.path.join(BASE_DIR, "glassdoor_service.lock")

logger = logging.getLogger("glassdoor_service")
...
def main():
    ...
    logger.info("Glassdoor Automation Service STARTED")
    ...
    try:
        while True:
            ...
            if now - last_pulse > 4 * 3600:
                logger.info("Executing Phase A: Glassdoor Pulse")
                try:
                    from glassdoor_auto_login import refresh_auth
                    if refresh_auth():
                        from glassdoor_update_manager import update_glassdoor_headline
                        from glassdoor_resume_uploader import upload_glassdoor_resume
            ...
            if 10 <= now_dt.hour < 13 and today != last_apply:
                logger.info("Executing Phase C: Glassdoor Smart Apply")
                try:
                    from glassdoor_auto_apply import run_auto_apply
                    run_auto_apply()
