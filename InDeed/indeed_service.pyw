"""
Indeed Full Automation Service (.pyw)
=====================================
...
LOCK_FILE = os.path.join(BASE_DIR, "indeed_service.lock")

logger = logging.getLogger("indeed_service")
...
def main():
    ...
    logger.info("Indeed Automation Service STARTED")
    ...
    try:
        while True:
            ...
            if now - last_pulse > 4 * 3600:
                logger.info("Executing Phase A: Indeed Pulse")
                try:
                    from indeed_auto_login import refresh_auth
                    if refresh_auth():
                        from indeed_update_manager import update_indeed_headline
                        from indeed_resume_uploader import upload_indeed_resume
                        update_indeed_headline()
                        upload_indeed_resume()
                        last_pulse = now
            ...
            if 10 <= now_dt.hour < 13 and today != last_apply:
                logger.info("Executing Phase C: Indeed Smart Apply")
                try:
                    from indeed_auto_apply import run_auto_apply
                    run_auto_apply()
